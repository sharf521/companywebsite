...
class Mysql {
	...
	**/
	function get_one($sql,$noreplace="")
	{	
		$_res=array();		
		$result = $this->query($sql,$noreplace);
	...
	* 获取全部的记录
	**/
	function get_all($sql,$noreplace="") 	
	{		
		$_res = array();		
		$result = $this->query($sql,$noreplace);
		$i = 0;		
  ...
  */
 function db_update($talbe, $dataArray, $where)
 {
	if (!is_array($dataArray) || count($dataArray)<=0)
	{
